...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    i, summation, x_points, y_points = 0, [], [], []
    length = len(polygon) - 1
    while i < length:
        sums = latitude(polygon[i]) * longitude(polygon[i + 1]) - latitude(polygon[i + 1]) * longitude(polygon[i])
        summation.append(sums)
        x_points.append((latitude(polygon[i]) + latitude(polygon[i + 1])) * sums)
        y_points.append((longitude(polygon[i]) + longitude(polygon[i + 1])) * sums)
        i = i + 1
    area = 0.5 * (sum(summation))
    if area == 0:
        return latitude(polygon[0]), longitude(polygon[0]), 0
    x_coordinate = sum(x_points) / (6 * area)
    y_coordinate = sum(y_points) / (6 * area)
...
def find_state_center(polygons):
    ...
    centroid_list = []
    for p in polygons:
        centroid_list.append(find_centroid(p))
    lat, lon, total_area = [], [], []
    for x, y, z, in centroid_list:
        lat.append(x * z), lon.append(y * z), total_area.append(z)
    x_point = sum(lat) / sum(total_area)
    y_point = sum(lon) / sum(total_area)
